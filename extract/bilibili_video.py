...
def down_video(video_list, title, start_url, page):
    ...
    currentVideoPath = os.path.join(sys.path[0], video_title, title)  # 当前目录作为下载目录
    ...
    for i in video_list:
        ...
        num += 1
    print('[正在下载P{}段视频下载完成]:'.format(page) + title)

# 合并视频(20190802新版)
def combine_video(title_list):
    video_path = os.path.join(sys.path[0], video_title)  # 下载目录
...
if __name__ == '__main__':
    ...
    if start.startswith("https") == False:  # 如果输入的是av号
        # 获取cid的api, 传入aid即可
        ...
        start_url = 'https://api.bilibili.com/x/web-interface/view?bvid=' + re.search(r'/BV(\d+)/*', start).group(1)
    ...
    currentVideoPath = os.path.join(sys.path[0],  video_title)  # 当前目录作为下载目录
